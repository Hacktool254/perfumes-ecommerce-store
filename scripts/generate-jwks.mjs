/**
 * Generate JWKS from the existing RSA private key.
 * 
 * This extracts the public key from the private key and formats it
 * as a JWKS (JSON Web Key Set) that Convex needs to verify JWTs.
 * 
 * Usage: node scripts/generate-jwks.mjs
 */
import { importPKCS8, exportJWK } from "jose";

// The private key from your .env.local (with literal \n replaced by actual newlines)
const privateKeyPem = `-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----`;

async function main() {
  try {
    // Import the private key
    const privateKey = await importPKCS8(privateKeyPem, "RS256");
    
    // Export the public portion as JWK
    const publicJwk = await exportJWK(privateKey);
    
    // Build the JWKS (JSON Web Key Set)
    const jwks = JSON.stringify({
      keys: [{ use: "sig", kty: publicJwk.kty, n: publicJwk.n, e: publicJwk.e }],
    });
    
    console.log("\n=== JWKS Value (copy this entire line) ===\n");
    console.log(jwks);
    console.log("\n=== End JWKS ===\n");
    
    console.log("\n=== Convex CLI command to set it ===\n");
    console.log(`npx convex env set JWKS '${jwks}'`);
    console.log("\n");
    
  } catch (error) {
    console.error("Error generating JWKS:", error);
    process.exit(1);
  }
}

main();
