/**
 * Fix PEM keys in Convex deployment.
 * The keys need proper newline formatting to work with RSA operations.
 */
import { execSync } from "child_process";

const PRIVATE_KEY = `-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----`;

function setEnvVar(name, value) {
  try {
    const result = execSync(`npx convex env set ${name}`, {
      input: value,
      encoding: "utf-8",
      stdio: ["pipe", "pipe", "pipe"],
      cwd: process.cwd(),
    });
    console.log(`✅ Set ${name} successfully`);
    return true;
  } catch (err) {
    console.error(`❌ Failed to set ${name}:`, err.stderr?.substring(0, 200) || err.message);
    return false;
  }
}

async function main() {
  console.log("Fixing PEM keys in Convex deployment...\n");
  
  // Set JWT_PRIVATE_KEY with proper newlines
  console.log("Setting JWT_PRIVATE_KEY...");
  setEnvVar("JWT_PRIVATE_KEY", PRIVATE_KEY);
  
  // Set CONVEX_AUTH_PRIVATE_KEY with proper newlines
  console.log("Setting CONVEX_AUTH_PRIVATE_KEY...");
  setEnvVar("CONVEX_AUTH_PRIVATE_KEY", PRIVATE_KEY);
  
  console.log("\nDone! Now redeploy Convex functions with: npx convex deploy");
}

main();
